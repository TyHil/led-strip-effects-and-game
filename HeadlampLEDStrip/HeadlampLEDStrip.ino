/*
  Headlamp WS2812B LED strip control for Arduino Nano with white light (with blue
...
#define NUM_LEDS 12
#define DATA_PIN 2
bool power = 1, fade = 1; //power starts off, fade in from off
...
void update() { //set all to correct brightness and blue light value
  ...
  else if (mode > 12 and mode < 16)
    setColor = CRGB(chosenColor.r * (mode == 13), chosenColor.g * (mode == 14), chosenColor.b * (mode == 15));
  else if (mode == 16) setColor = chosenColor;
  if (mode != 11 and mode != 12)
    for (uint8_t i = 0; i < NUM_LEDS; i++) leds[i] = setColor;
}
void setup() {
...
void loop() {
  if (irrecv.decode(&results)) {
    ...
    if (results.value == 0xE0E040BF) { //on/off
      ...
      if (power) {
        ...
        for (int16_t i = (brightness - brightness / 17) * (mode < 13 or mode > 15) + 238 * (mode > 12 and mode < 16);
             i > 0;
             i -= (brightness / 17) * (mode < 13 or mode > 15) + 17 * (mode > 12 and mode < 16)) {
          FastLED.show(i);
          delay(40);
        }
        FastLED.clear();
  ...
  if (fade) { //fade in
    fade = false;
    for (int16_t i = (brightness / 17) * (mode < 13 or mode > 15) + 17 * (mode > 12 and mode < 16);
         i <= brightness * (mode < 13 or mode > 15) + 255 * (mode > 12 and mode < 16);
         i += (brightness / 17) * (mode < 13 or mode > 15) + 17 * (mode > 12 and mode < 16)) {
      FastLED.show(i);
      delay(40);
    }
